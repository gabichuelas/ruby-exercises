...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_words_containing_e
    ...
    selected = []
    words.each { |word|
      selected << word unless word.match(/e/)}
  ...
  def test_remove_dinosaurs
    ...
    notasaurus = []
    animals.each { |word|
      notasaurus << word unless word.match(/saurus/)}
  ...
  def test_remove_numbers
    ...
    not_numbers = []
    elements.each { |obj|
      not_numbers << obj unless obj.class == Integer || obj.class == Float }
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each { |elem|
      not_numbers << elem unless elem.class == Float }
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each { |animal|
      remaining << animal unless animal[0].match(/[aeiou]/) }
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each { |obj|
      remaining << obj unless obj.class == Array }
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each { |obj|
      remaining << obj unless obj.class == Hash }
