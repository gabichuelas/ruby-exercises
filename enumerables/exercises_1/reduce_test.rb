...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    # skip
    numbers = [2, 3, 5, 7]
    result = numbers.reduce(1) { |multiply, num|
      multiply *= num }
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    ...
    result = keywords.reduce('one fish two fish red fish blue fish') { |phrase, word|
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) { |difference, ary| difference -= ary.min }
  ...
  def test_add_all_the_second_values_together
    ...
    # initial value is 0
    result = elements.reduce(0) { |sum, ary|
      sum += ary[1] }
